 ...
 *  - All package-private helpers (formattedStatValuePairs, outliers,
...
public class ReporterTest {
    ...
    // =========================================================================
    // outliers(double[])   <-- package-private method
    // =========================================================================

    @Test
    // outliers() | Edge: empty array → "No outliers."
    void outliers_emptyArray_returnsNoOutliersMessage() {
        Reporter reporter = new Reporter();
        assertEquals("No outliers.", reporter.outliers(new double[]{}));
    }

    @Test
    // outliers() | Happy: single outlier → starts with "Outliers:" and includes value.
    void outliers_singleOutlier_returnsOutliersMessage() {
        Reporter reporter = new Reporter();
        String result = reporter.outliers(new double[]{5.0});
    ...
    @Test
    // outliers() | Happy: multiple outliers → all values appear in the string.
    void outliers_multipleOutliers_allValuesPresent() {
        Reporter reporter = new Reporter();
        String result = reporter.outliers(new double[]{1.0, 99.0, 200.0});
    ...
    @Test
    // outliers() | Equivalence: non-empty array never returns "No outliers."
    void outliers_nonEmptyArray_neverReturnsNoOutliersMessage() {
        Reporter reporter = new Reporter();
        String result = reporter.outliers(new double[]{42.0});
        assertFalse(result.contains("No outliers."));
    }

    @Test
    // outliers() | Path: enough outliers to push getNumberArrayString past
    // the 72-char limit → truncated "and N more" notation must appear.
    void outliers_manyOutliers_truncatesWhenExceeds72CharLimit() {
        Reporter reporter = new Reporter();
        ...
        String result = reporter.outliers(manyOutliers);
        assertTrue(result.startsWith("Outliers:"), "Must start with 'Outliers:'");
